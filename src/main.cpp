...
uint256 hashGenesisBlock("000000000e8814d58fe7834fde76e8aa000972894b49fae4cbfbd5c8c9005a9c");
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Limit adjustment step, 15% up or down
    int64 nActualTimespan = pindexLast->GetBlockTime() - pindexFirst->GetBlockTime();
    printf("  nActualTimespan = %"PRI64d"  before bounds\n", nActualTimespan);
    if (nActualTimespan < ((nTargetTimespan*55)/65))
        nActualTimespan = ((nTargetTimespan*55)/65);
    if (nActualTimespan > ((nTargetTimespan*75)/65))
        nActualTimespan = ((nTargetTimespan*75)/65);
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block        
        const char* pszTimestamp = "The Times 11/Apr/2013 Sleep, Sleep, Sleeeeep!!!!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dfa939b73ee3f16103ffa6442a2948ee8ef615bd0b5e43c50b2a16f2c3aa535ae5414f7f14b8e431ff98129815d95b07dd7c8189ce7bd116508850641935447e") << OP_CHECKSIG;
        CBlock block;
        ...
        block.nTime    = 1365638399;
        block.nNonce   = 3268587783;
        ...
        assert(block.hashMerkleRoot == uint256("0x908095094c1f0886ab5e81c1646e69c9864e6d23f997e6db4b9f4fae506be75c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xbc, 0xac, 0xd4 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

unsigned int static ScanHash_CryptoPP(CBlock* tBlock, unsigned char* phash, unsigned int& nHashesDone)
{
    for (;;)
    {
        tBlock->nNonce = tBlock->nNonce + 1;
        uint256 h = tBlock->GetHash();
        memcpy(phash, &h, 32);
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running eCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            // SHA3 (NonOptimized)
            nNonceFound = ScanHash_CryptoPP(pblock, (unsigned char*)&hash, nHashesDone);            
            ...
            {                
                ...
                {
                    // Found a solution                                        
                    assert(pblock->nNonce == nNonceFound);
            ...
            // Update nTime everyStartMiningAsStartup few seconds
            // pblock->UpdateTime(pindexPrev);
        }
    }
}
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadeCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "eCoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadeCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d eCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBitcoinMiner, pwallet))
                printf("Error: NewThread(ThreadeCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
